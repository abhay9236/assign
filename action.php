...
if(isset($_POST['add'])){
    ...
    $query="INSERT INTO crud2(name,email,phone,photo)VALUES(?,?,?,?)";
...
{
    $id=$_GET['delete'];
    $sql="SELECT photo FROM crud2 WHERE id=?";
...
if(isset($_GET['edit'])){
    $id=$_GET['edit'];
    $query="SELECT * FROM crud2 WHERE id=?";
    $stmt=$conn->prepare($query);
    $stmt->bind_param("i",$id);
    $stmt->execute();
...
$query="UPDATE crud2 SET name =?,email=?,phone=?,photo=? WHERE id=?";
$stmt=$conn->prepare($query);
